...
SELECT p.Product_Name, r.Review_Text, r.Review_Date, c.Customer_Name
...
FROM Product p JOIN Rating rt ON p.Product_ID = rt.Product_ID
GROUP BY p.Product_ID, p.Product_Name;

SELECT p.Product_Name, ROUND(AVG(rt.Rating_Value), 2) AS Average_Rating
FROM Product p JOIN Rating rt ON p.Product_ID = rt.Product_ID
GROUP BY p.Product_ID, p.Product_Name
