...
- One Order can contain many Order_Details records.
- Each Order_Details record references one Product.
- `Orders.Customer_ID` references `Customer.Customer_ID`.
- `Order_Details.Order_ID` references `Orders.Order_ID`.
- `Order_Details.Product_ID` references `Product.Product_ID`.
...
VALUES (1, 19999.00, 'Placed');

INSERT INTO Order_Details (Order_ID, Product_ID, Quantity, Unit_Price)
VALUES (1, 1, 1, 19999.00);

UPDATE Orders
SET Status = 'Confirmed', Total_Amount = 19999.00
WHERE Order_ID = 1;
...
> Run Task 1 and Task 2 before Task 4 because Orders references Customer and Order_Details references Product.
